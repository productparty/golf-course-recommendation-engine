...
import { Container, Typography, TextField, Button, MenuItem, Select, InputLabel, FormControl, Box, SelectChangeEvent, Grid } from '@mui/material';
  ...
  const fetchCourses = async (pageNumber: number) => {
    try {
      ...
      const response = await axios.get<{ results: GolfCourse[]; total: number }>(
        'http://localhost:8000/find_courses/', // Ensure this URL is correct
        { params }
      );
  ...
  return (
    <div className="find-course-container">
      ...
      <div className="top-image">
        <img
          src="/public/golfcourseheader.jpg" // Replace with your image path
              ...
              <ul className="results">
                {results.slice(0, 10).map((course, idx) => (
                  <li key={idx} className="result-item">
                    <Typography variant="h6">{course.club_name} - {course.course_name}</Typography>
                    <Typography variant="body2">{course.city}, {course.state} ({course.distance_miles.toFixed(2)} miles)</Typography>
                  </li>
                ))}
              </ul>
